...
koji add-tag build --arches=x86_64,amd64
koji add-tag dest --arches=x86_64,amd64
...
token_x86_64=$(oc whoami -t)

oc new-project worker-amd64
oc adm policy add-role-to-user edit -z builder
...
oc create policybinding worker-amd64
...
oc adm policy add-role-to-user osbs-custom-build osbs -z builder --role-namespace worker-amd64

oc secrets new-dockercfg v2-registry-dockercfg --docker-server=172.17.0.1:5000 --docker-username=osbs --docker-password=craycray --docker-email=test@test.com

token_amd64=$(oc whoami -t)

sed -i "s/OSBS_x86_64_TOKEN/${token_x86_64}/" /tmp/osbs.conf
sed -i "s/OSBS_amd64_TOKEN/${token_amd64}/" /tmp/osbs.conf
